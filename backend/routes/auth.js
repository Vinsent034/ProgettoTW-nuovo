...
router.post('/register', async (req, res) => {
    try {
        ...
        if (utenteEsiste) { // controllo per capire se riguarda essitenza omeno
            return res.status(400).json({ error: 'Email giÃ  registrata. Effettua il login.' });
        }
    ...
    // ultimo conterollo estenuante di sicurezza
        ...
        // errori dal databaseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
...
router.post('/login', async (req, res) => {
    try {
        const { email, password } = req.body; // recupero i dati 
