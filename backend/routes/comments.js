...
router.get('/cat/:catId', async (req, res)=>{
    try{
            ...
            const commenti = await Commenti.find({cat : req.params.catId}).populate('author', 'name email').sort({date : -1});
...
router.post('/:catId', autenticate, async (req, res)=>{
    try{
            const { text } = req.body;
    ...
    router.delete('/:commentId', autenticazione, async (req, res)=>{
