...
function CaricaGatti() {
  ...
  fetch(backend + '/cats') .then(response => response.json()).then(gatti => {
  ...
  // contrasegni ogni fatto con un amker 
   ...
   // NOTA se metto lo stile nel CSS scoppia
   const popupContent = `
...
function CreaMappa(){
      mappa = L.map('map').setView([40.8518, 14.2681], 13); // e la posizione di napoli
...
function Logout() { // da qua rimuvo l'utente log e ricarica
